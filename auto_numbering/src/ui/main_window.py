...
class MainWindow:
    ...
    def _step4_execute(self) -> None:
        ...
        if self.number_config.get("merge_overlap"):
            ...
            if overlap_groups:
                ...
                warn_lbl.pack(anchor=tk.W, pady=(0, 5), before=minimap_frame)
        ...
        # minimap_frame 内部用 grid：canvas 占满，按钮固定底边
        minimap_frame.grid_rowconfigure(0, weight=1)
        minimap_frame.grid_columnconfigure(0, weight=1)
        self._minimap_canvas = tk.Canvas(
            minimap_frame, bg="#fefefe",
            highlightthickness=1, highlightbackground="#ddd")
        self._minimap_canvas.grid(row=0, column=0, sticky="nsew", padx=2, pady=2)
        # 缓存类型值（避免每次点击调 COM）
        self._cached_types = {}
        for i, blk in enumerate(self._reorder_blocks):
            try:
                self._cached_types[i] = AcadConnect.get_attribute_value(blk, self.type_tag) or "?"
            except Exception:
                self._cached_types[i] = "?"
        # 左键拖拽平移 + 点击选中
        self._minimap_canvas.bind("<Button-1>", self._on_minimap_press)
        ...
        # canvas 自身 <Configure> 事件（尺寸变化时触发重绘，防抖处理）
        self._minimap_canvas.bind("<Configure>",
            lambda e: self._schedule_minimap_redraw())
        # 窗口级 resize 处理（只绑一次）
        if not getattr(self, '_root_configure_bound', False):
            self.root.bind("<Configure>", lambda e: self._schedule_minimap_redraw(), add="+")
            self._root_configure_bound = True

        # ── 操作说明（grid row=1）──
        hint_frame = ttk.Frame(minimap_frame)
        hint_frame.grid(row=1, column=0, sticky="ew", pady=(2, 0))
        tk.Label(hint_frame,
            text="① 单击圆圈=选中  Ctrl+单击=多选  ② 同位置编组=同位置两块同号  顺号编组=所选块+A/B/C后缀",
            font=("微软雅黑", 8), fg="#999", anchor=tk.W).pack(side=tk.LEFT)
        tk.Label(hint_frame,
            text="  ③ 点↑↓=互换整个编组编号  ↩拆散编组=取消编组",
            font=("微软雅黑", 8), fg="#999", anchor=tk.W).pack(side=tk.LEFT, padx=(10, 0))

        # ── 调序控制栏（grid row=2）──
        ctrl_frame = ttk.Frame(minimap_frame)
        ctrl_frame.grid(row=2, column=0, sticky="ew", pady=(2, 0))
        ...
        # 上移/下移（按编号单元整体移动）
        self._group_up_btn = ttk.Button(
            ctrl_frame, text="↑ 上移",
            command=self._group_move_up, width=7, state=tk.DISABLED)
        self._group_up_btn.pack(side=tk.LEFT, padx=(0, 3))

        self._group_down_btn = ttk.Button(
            ctrl_frame, text="↓ 下移",
            command=self._group_move_down, width=7, state=tk.DISABLED)
        self._group_down_btn.pack(side=tk.LEFT, padx=(0, 3))

        # ── 分隔线 ──
        ttk.Separator(ctrl_frame, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=(0, 5))

        self._merge_btn = ttk.Button(
            ctrl_frame, text="🔗合并编号",
            command=self._merge_selected, width=10, state=tk.DISABLED)
        self._merge_btn.pack(side=tk.LEFT, padx=(0, 3))
        ...
        self._exec_label.pack(anchor=tk.W, pady=(4, 0))
    ...
    def _compute_preview(self) -> None:
        ...
        try:
            ...
            # 预构建 merge_map 的分组索引，方便按组查
            mgroups_by_mid: Dict[int, List[int]] = {}
            if merge_map:
                for idx, mid in merge_map.items():
                    mgroups_by_mid.setdefault(mid, []).append(idx)
            ...
            for i in range(n):
                ...
                if merge_map and i in merge_map:
                    mid = merge_map[i]
                    # 找出该合并组所有还没分配的成员
                    all_mg = [j for j in range(n)
                              if merge_map.get(j) == mid
                              and j not in assigned]
                    # 如果合并组里混有手动组成员 → 整个留给 ①处理
                    if any(j in gm for j in all_mg):
                        continue
                    for m in all_mg:
                        assigned.add(m)
                    eff_groups.append(all_mg)
                    continue
            ...
            # ── 保存 eff_groups + inherit + merge_map 供组调序用 ──
            self._eff_groups = eff_groups
            self._inherit_map = inherit
            if merge_map:
                self._merge_map = merge_map
            # 任何修改导致预览重算 → 重置编号完成状态，按钮变回「开始编号」
            if getattr(self, '_exec_done', False):
                self._exec_done = False
                self._update_buttons()
                ...
                for m in group:
                    ...
                    if m in in_mg:
                        for mems in valid_mg.values():
                            if m in mems:
                                # 合并对在手动组中应共享同一后缀
                                if merge_map and m in merge_map:
                                    unique = sorted(set(merge_map.get(x, x) for x in mems))
                                    s_pos = unique.index(merge_map[m])
                                else:
                                    s_pos = mems.index(m)
                                suffix = chr(ord('A') + s_pos) if s_pos < 26 else f"_{s_pos}"
            ...
            self._reorder_preview = {}
        # 调试日志：输出每个分组的编号结果
        if self._reorder_preview:
            debug_lines = [f"eff_groups({len(eff_groups)}组):"]
            for pos, g in enumerate(eff_groups):
                tags = [f"{self._reorder_preview.get(m,'?')}" for m in g]
                debug_lines.append(f"  pos={pos} idx{g} → {tags}")
            write_log("\n".join(debug_lines))
    ...
    def _render_minimap(self) -> None:
        ...
        # canvas 高度 = 窗口高度 - 底部固定 UI
        win_h = self.root.winfo_height()
        ch = max(win_h - 300, 200)
        try:
        ...
        for i, (blk, pt) in enumerate(zip(blocks, points)):
            ...
            if is_sel:
                # 选中：大号发光效果
                sr = r + 6
                glow = canvas.create_oval(cx - sr, cy - sr, cx + sr, cy + sr,
                                          outline="#fc0", width=3,
                                          tags=(f"glow_{i}",))
                oid = canvas.create_oval(cx - r - 1, cy - r - 1, cx + r + 1, cy + r + 1,
                                         fill=color, outline="#fc0", width=3,
                                         tags=(f"blk_{i}",))
            else:
                glow = None
            ...
            # 文字颜色：合并对=金色，手动组=浅蓝，普通=白
            txt_color = "white"
            mm = getattr(self, '_merge_map', {})
            if i in mm:
                txt_color = "#ffd700"
            elif i in gm:
                txt_color = "#87ceeb"
            canvas.create_text(cx, cy, text=short, fill=txt_color,
                               font=("Arial", 10, "bold"),
                               tags=(f"blk_{i}",))
            canvas.create_text(cx, cy + r + 10,
                               text=getattr(self, '_cached_types', {}).get(i, "?"),
                               fill="#555", font=("Arial", 8),
                               tags=(f"blk_{i}",))
            self._minimap_items[i] = {"oval": oid, "glow": glow,
                                       "cx": cx, "cy": cy, "r": r}
    ...
    def _refresh_minimap_selection(self, changed_only: set = None) -> None:
        """轻量更新选中高亮（只改边框，不删重建）
        Args:
            changed_only: 只更新指定索引，None=更新全部
        """
        canvas = self._minimap_canvas
        items = getattr(self, '_minimap_items', {})
        sel_set = getattr(self, '_selected_indices', set())
        iter_items = items.items() if changed_only is None else [(i, items[i]) for i in changed_only if i in items]
        for i, ids in iter_items:
            is_sel = i in sel_set
            if not ids.get("oval"):
                continue
            try:
                ...
                if is_sel:
                    # 选中：金色边框
                    canvas.itemconfig(ids["oval"], outline="#fc0", width=3)
                    if not ids.get("glow"):
                        sr = r + 6
                    ...
                    # 未选中：灰色边框，删光晕
                    canvas.itemconfig(ids["oval"], outline="#555", width=1)
                    if ids.get("glow"):
    ...
    def _render_reorder_panel(self) -> None:
        ...
        for r_start, r_end in rows:
            ...
            for i in range(r_start, r_end + 1):
                tv = getattr(self, '_cached_types', {}).get(i, "?")
                # 编号预览
                ...
                is_sel = i in getattr(self, '_selected_indices', set())
                item_bg = "#fef5e7" if is_sel else "#fafafa"
                item_f = tk.Frame(self._reorder_inner, bg=item_bg)
    ...
    def _on_minimap_drag(self, event):
        ...
        if abs(dx) > 3 or abs(dy) > 3:
            ...
            if cw > 0:
                new_first = max(0.0, min(1.0 - span_x, vx[0] - dx / cw))
                canvas.xview_moveto(new_first)
            if ch > 0:
                new_first = max(0.0, min(1.0 - span_y, vy[0] - dy / ch))
    ...
    def _on_minimap_click(self, event):
        ...
        if item:
            ...
            for tag in tags:
                ...
                if tag.startswith("glow_"):
                    ...
                    break
        # 记录旧选中，只更新有变化的块
        old_sel = set(self._selected_indices)
        if idx >= 0:
            if ctrl:
        ...
        # 只更新变化的块（旧+新）
        changed = old_sel | self._selected_indices
        self._refresh_minimap_selection(changed)
        self._update_controls()
    ...
    def _update_controls(self) -> None:
        ...
        if sel:
            idx = next(iter(sel))
            n = len(sel)
            tv = getattr(self, '_cached_types', {}).get(idx, "?")
            ...
            enabled = tk.NORMAL
            # 合并编号按钮：选中 ≥2 个即可
            self._merge_btn.config(state=tk.NORMAL if n >= 2 else tk.DISABLED)
        ...
        # 上移/下移按钮：选中且在 eff_groups 存在时启用
        has_eg = hasattr(self, '_eff_groups') and self._eff_groups
        eg_state = tk.NORMAL if (sel and has_eg and n == 1) else tk.DISABLED
        self._group_up_btn.config(state=eg_state)
        self._group_down_btn.config(state=eg_state)

    def _group_move_up(self) -> None:
        """将选中块所在的 eff_group 上移一位（只改编号，不改块顺序）"""
        if not self._selected_indices or not hasattr(self, '_eff_groups'):
            return
        idx = next(iter(self._selected_indices))
        # 找到 idx 所在的 eff_group
        for pos, group in enumerate(self._eff_groups):
            if idx in group:
                if pos == 0:
                    return  # 已经是第一个，不能上移
                # 交换
                self._eff_groups[pos], self._eff_groups[pos - 1] = \
                    self._eff_groups[pos - 1], self._eff_groups[pos]
                self._reassign_from_eff_groups()
                self._render_minimap()
                self._update_controls()
                cfg = self.number_config
                self.log(f"↑组: 编组从 {cfg.get('prefix','')}{cfg.get('start_number',1)+pos-1} "
                         f"交换到 {cfg.get('prefix','')}{cfg.get('start_number',1)+pos}", "SUCCESS")
                return

    def _group_move_down(self) -> None:
        """将选中块所在的 eff_group 下移一位（只改编号，不改块顺序）"""
        if not self._selected_indices or not hasattr(self, '_eff_groups'):
            return
        idx = next(iter(self._selected_indices))
        for pos, group in enumerate(self._eff_groups):
            if idx in group:
                if pos == len(self._eff_groups) - 1:
                    return  # 已经是最后一个
                self._eff_groups[pos], self._eff_groups[pos + 1] = \
                    self._eff_groups[pos + 1], self._eff_groups[pos]
                self._reassign_from_eff_groups()
    ...
    def _reassign_from_eff_groups(self) -> None:
        """根据 self._eff_groups 的当前顺序重新分配编号"""
        if not hasattr(self, '_eff_groups') or not self._eff_groups:
            return
        cfg = self.number_config if hasattr(self, 'number_config') and self.number_config else {}
        start_num = cfg.get("start_number", 1)
        prefix = cfg.get("prefix", "")
        ...
        for m in valid_mg.values():
            in_mg.update(m)
        # 继承映射（从 _compute_preview 中的 inherit）
        inherit = getattr(self, '_inherit_map', {})

        self._reorder_preview = {}
        merge_map = getattr(self, '_merge_map', None)
        for pos, group in enumerate(self._eff_groups):
            for m in group:
                num = start_num + pos
                num_str = str(num).zfill(pad_width) if pad_width > 0 else str(num)
    ...
    def _merge_selected(self) -> None:
        ...
        if len(self._selected_indices) < 2:
            return
        sel_blocks = [self._reorder_blocks[i] for i in sorted(self._selected_indices)]
        from src.numbering import NumberingEngine
        groups = NumberingEngine._group_by_proximity(
            sel_blocks,
            merge_distance=self.number_config.get("merge_distance", 20.0),
            type_tag=self.type_tag)
        next_neg = -1
        merged = 0
        for group in groups:
            if len(group) < 2:
                continue
            # 将组索引映射回原始 _reorder_blocks 索引
            orig_indices = [sorted(self._selected_indices)[i] for i in group]
            for idx in orig_indices:
                self._group_map[idx] = next_neg
            next_neg -= 1
            merged += len(orig_indices)
        if merged:
            self._compute_preview()
            self._render_minimap()
            self._update_controls()
            self.log(f"🔗 合并编号: {merged} 个块合并为 {abs(next_neg)-1} 组", "SUCCESS")
